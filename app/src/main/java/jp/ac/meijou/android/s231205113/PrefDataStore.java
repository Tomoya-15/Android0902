...
public class PrefDataStore {
    ...
    public Optional<String> getString(String key) {
        return dataStore.data()
                .map(prefs -> {
                    var prefKey = PreferencesKeys.stringKey(key); // 入れている型にあったPreferences.Keyのインスタンスを作成
                    return Optional.ofNullable(prefs.get(prefKey)); // nullを返すとクラッシュするのでOptionalでラップする
                })
                .blockingFirst(); // RxJavaで最初の値を返す
    }
}
